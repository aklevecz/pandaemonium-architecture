                                         ...
                                         L ARGE S CALE GAN T RAINING FOR
                                         H IGH F IDELITY NATURAL I MAGE S YNTHESIS
                                          Andrew Brock∗ †                 Jeff Donahue†                          Karen Simonyan†
                                          Heriot-Watt University          DeepMind                               DeepMind
                                          ajb5@hw.ac.uk                   jeffdonahue@google.com                 simonyan@google.com
...
arXiv:1809.11096v2 [cs.LG] 25 Feb 2019
                                                     ...
                                                     (BigGANs) achieve an Inception Score (IS) of 166.5 and Fréchet Inception Dis-
...
Fréchet Inception Distance (FID) from 52.52 and 18.65 to 166.5 and 7.4 respectively. We also
...
Published as a conference paper at ICLR 2019
    ...
    Batch   Ch.   Param (M)   Shared Skip-z Ortho.         Itr ×103         FID              IS
     ...
     512    64       81.5       7      7         7           1000          15.30        58.77(±1.18)
    1024    64       81.5       7      7         7           1000          14.88        63.03(±1.42)
    2048    64       81.5       7      7         7            732          12.39        76.85(±3.83)
    2048    96      173.5       7      7         7        295(±18)      9.54(±0.62)     92.98(±4.27)
    2048    96      160.6       3      7         7        185(±11)      9.18(±0.13)     94.94(±1.32)
    2048    96      158.3       3      3         7         152(±7)      8.73(±0.45)     98.76(±2.84)
    2048    96      158.3       3      3        3         165(±13)      8.51(±0.32)     99.31(±2.10)
    2048    64       71.3       3      3        3          371(±7)     10.48(±0.10)     86.90(±0.61)

Table 1: Fréchet Inception Distance (FID, lower is better) and Inception Score (IS, higher is better)
...
(Salimans et al., 2016; Heusel et al., 2017; Bińkowski et al., 2018; Wu et al., 2017). Of these,
the Inception Score (IS, Salimans et al. (2016)) and Fréchet Inception Distance (FID, Heusel et al.
...
3.1   T RADING OFF VARIETY AND FIDELITY WITH THE T RUNCATION T RICK
...
filters but does not constrain their norm:
                                 ...
                                 Rβ (W ) = βkW > W         (1 − I)k2F ,                              (3)
                                                                                             −4
where 1 denotes a matrix with all elements set to 1. We sweep β values and select 10 , finding
...
4.1   C HARACTERIZING I NSTABILITY: T HE G ENERATOR
...
either σ0 or σσ01 , but even though in some cases they mildly improve performance, no combination
...
4.2   C HARACTERIZING I NSTABILITY: T HE D ISCRIMINATOR
...
(with further plots in Appendix F). Unlike G, we see that the spectra are noisy, σσ01 is well-behaved,
...
5.1      E VALUATION ON I MAGE N ET
...
Published as a conference paper at ICLR 2019
      ...
      Ch.   Param (M)   Shared   Skip-z    Ortho.    FID      IS      (min FID) / IS   FID / (max IS)
       64     317.1       7        7         7      48.38    23.27      48.6/23.1        49.1/23.9
       64      99.4       3        3         3      23.48    24.78      22.4/21.0        60.9/35.8
       96     207.9       3        3         3      18.84    27.86      17.1/23.3        51.6/38.1
      128     355.7       3        3         3      13.75    30.61      13.0/28.0        46.2/47.8
...
5.2     A DDITIONAL EVALUATION ON JFT-300M
...
Martin Arjovsky, Soumith Chintala, and Léon Bottou. Wasserstein generative adversarial networks.
 In ICML, 2017.
...
Marc G. Bellemare, Ivo Danihelka, Will Dabney, Shakir Mohamed, Balaji Lakshminarayanan,
 Stephan Hoyer, and Rémi Munos. The Cramer distance as a solution to biased Wasserstein gra-
 dients. In arXiv preprint arXiv:1705.10743, 2017.

Mikolaj Bińkowski, Dougal J. Sutherland, Michael Arbel, and Arthur Gretton. Demystifying MMD
 GANs. In ICLR, 2018.
...
Harm de Vries, Florian Strub, Jérémie Mary, Hugo Larochelle, Olivier Pietquin, and Aaron
...
Ishaan Gulrajani, Faruk Ahmed, Martı́n Arjovsky, Vincent Dumoulin, and Aaron C. Courville. Im-
   proved training of Wasserstein GANs. In NIPS, 2017.
Kaiming He, Xiangyu Zhang, Shaoqing Ren, and Jian Sun. Deep residual learning for image recog-
  nition. In CVPR, 2016.
Martin Heusel, Hubert Ramsauer, Thomas Unterthiner, Bernhard Nessler, Günter Klambauer, and
...
Lucas Theis, Aäron van den Oord, and Matthias Bethge. A note on the evaluation of generative
...
A PPENDIX A       A DDITIONAL S AMPLES , I NTERPOLATIONS , AND N EAREST
                  N EIGHBORS FROM I MAGE N ET MODELS
...
A PPENDIX B               A RCHITECTURAL DETAILS
...
use attention at 64 × 64 resolution, channel width multiplier ch = 128, and z ∈ R128 .
     ...
     → 4x4x16ch                                  ReLU
      ...
      ResBlock          Concat      Upsample   Upsample                Concat                     ReLU


                                    1x1 Conv   3x3 Conv                         1x1 Conv         3x3 Conv
      ResBlock          Concat

                                               BatchNorm      Linear            Average
                                                                                                  ReLU
      Non-local                                                                 Pooling
...
A PPENDIX C         E XPERIMENTAL D ETAILS
...
C.1   BATCH N ORM S TATISTICS AND S AMPLING
...
C.3   I NCEPTION S CORES OF I MAGE N ET I MAGES
...
A PPENDIX D        A DDITIONAL P LOTS
...
Published as a conference paper at ICLR 2019
                                 ...
                                 0                                 FID vs IS as a function of truncation
                                                                                                                                Ch=128
                                                                                                                                Ch=96
                                                                                                                                Ch=64
                                                                                                                                Ch=64 (Baseline)
                 ...
                 JFT-300M FID
                                ...
                                10                                 FID vs IS as a function of truncation
                                                                                                                                Ch=128
                                                                                                                                Ch=96
                                                                                                                                Ch=64
                                                                                                                                Ch=64 (Baseline)
...
A PPENDIX E        C HOOSING L ATENT S PACES
...
L ATENTS
      ...
      • Per-sample variable variance: N (0, σi I), where σi ∼ U[σl , σh ] independently for each
...
A PPENDIX F       M ONITORED T RAINING S TATISTICS
...
A PPENDIX G         A DDITIONAL D ISCUSSION : S TABILITY AND C OLLAPSE
...
G.1    I NTERVENING B EFORE C OLLAPSE
...
G.2    S PIKES IN THE D ISCRIMINATOR ’ S S PECTRA
...
degrades training.
   ...
   3
     Unconstrained models could easily learn a different output scale to account for this margin, but the use of
...
A PPENDIX H         N EGATIVE R ESULTS
