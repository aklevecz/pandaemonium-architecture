...
The high dimensional nature of images presents distinct challenges to generative modeling. Generative Adversarial Networks (GAN) [27] allow for efficient sampling of high resolution images with good perceptual quality [3, 42], but are diffi-
...
| LDM-4 (ours, 100 steps) | 2.8$^\dagger$/4.8$^\ddagger$ | 166.3 | 24.4±3.8 | 0.69±0.14 | 169M | 4.62 |
| emphLDM-4 (ours, big, 100 steps) | 2.4$^\dagger$/4.3$^\ddagger$ | 174.9 | 24.7±4.1 | 0.71±0.15 | 552M | 4.5 |
| LDM-4 (ours, 50 steps, guiding) | 4.4$^\dagger$/6.4$^\ddagger$ | 153.7 | 25.8±3.7 | 0.74±0.12 | 184M | 0.38 |
...
| Method | 40-50% masked FID ↓ | LPIPS ↓ | All samples FID ↓ | LPIPS ↓ |
| :--- | :--- | :--- | :--- | :--- |
...
[6] Mark Chen, Alec Radford, Rewon Child, Jeffrey Wu, Hee-woo Jun, David Luan, and Ilya Sutskever. Generative pretraining from pixels. In ICML, volume 119 of Proceedings of Machine Learning Research, pages 1691–1703. PMLR, 2020. 3
...
[17] Ming Ding, Zhuoyi Yang, Wenyi Hong, Wendi Zheng, Chang Zhou, Da Yin, Junyang Lin, Xu Zou, Zhou Shao, Hongxia Yang, and Jie Tang. Cogview: Mastering text-to-image generation via transformers. CoRR, abs/2105.13290, 2021. 6, 7
...
[50] Tuomas Kynkäänniemi, Tero Karras, Samuli Laine, Jaakko Lehtinen, and Timo Aila. Improved precision and recall metric for assessing generative models. *CoRR*, abs/1904.06991, 2019. 5, 26
...
[53] Chenlin Meng, Yang Song, Jiaming Song, Jiajun Wu, Jun-Yan Zhu, and Stefano Ermon. Sdedit: Image synthesis and editing with stochastic differential equations. *CoRR*, abs/2108.01073, 2021. 1
...
[99] Thomas Wolf, Lysandre Debut, Victor Sanh, Julien Chaumond, Clement Delangue, Anthony Moi, Pierric Cistac, Tim Rault, Rémi Louf, Morgan Funtowicz, and Jamie Brew. Huggingface’s transformers: State-of-the-art natural language processing. CoRR, abs/1910.03771, 2019. 26
...
[108] Bolei Zhou, Àgata Lapedriza, Aditya Khosla, Aude Oliva, and Antonio Torralba. Places: A 10 million image database for scene recognition. IEEE Transactions on Pattern Analysis and Machine Intelligence, 40:1452–1464, 2018. 8, 9, 26
...
Diffusion models can be specified in terms of a signal-to-noise ratio $\text{SNR}(t) = \frac{\alpha_t^2}{\sigma_t^2}$ consisting of sequences $(\alpha_t)_{t=1}^T$ and $(\sigma_t)_{t=1}^T$ which, starting from a data sample $x_0$, define a forward diffusion process $q$ as

$$q(x_t | x_0) = \mathcal{N}(x_t | \alpha_t x_0, \sigma_t^2 \mathbf{I}) \quad (4)$$

with the Markov structure for $s < t$:

$$q(x_t | x_s) = \mathcal{N}(x_t | \alpha_{t|s} x_s, \sigma_{t|s}^2 \mathbf{I}) \quad (5)$$
...
$$-\log p(x_0) \leq \text{KL}(q(x_T | x_0) | p(x_T)) + \sum_{t=1}^T \mathbb{E}_{q(x_t | x_0)} \text{KL}(q(x_{t-1} | x_t, x_0) | p(x_{t-1} | x_t)) \quad (9)$$
...
$$= \mathcal{N}(x_{t-1} | \mu_\theta(x_t, t), \sigma_{t|t-1}^2 \frac{\sigma_{t-1}^2}{\sigma_t^2} \mathbf{I}), \quad (11)$$
...
$$\sum_{t=1}^{T} \mathbb{E}_{q(x_t|x_0)} \mathbb{KL}(q(x_{t-1}|x_t, x_0)|p(x_{t-1})) = \sum_{t=1}^{T} \mathbb{E}_{\mathcal{N}(\epsilon|0, \mathbb{I})} \frac{1}{2} (\mathbf{SNR}(t-1) - \mathbf{SNR}(t)) \|x_0 - x_\theta(\alpha_t x_0 + \sigma_t \epsilon, t)\|^2 \quad (13)$$
...
As discussed in Sec. 4.3.2, the signal-to-noise ratio induced by the variance of the latent space (i.e. Var(z)/$\sigma_t^2$) significantly affects the results for convolutional sampling. For example, when training a LDM directly in the latent space of a KL-regularized model (see Tab. 8), this ratio is very high, such that the model allocates a lot of semantic detail early on in the reverse denoising process. In contrast, when rescaling the latent space by the component-wise standard deviation of the latents as described in Sec. G, the SNR is descreased. We illustrate the effect on convolutional sampling for semantic image synthesis in Fig. 15. Note that the VQ-regularized space has a variance close to 1, such that it does not have to be rescaled.
...
| $f$ | $|Z|$ | $c$ | **R-FID** $\downarrow$ | **R-IS** $\uparrow$ | **PSNR** $\uparrow$ | **PSIM** $\downarrow$ | **SSIM** $\uparrow$ |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
| Method | COCO256 × 256 FID↓ | OpenImages 256 × 256 FID↓ | OpenImages 512 × 512 FID↓ |
| :--- | :--- | :--- | :--- |
| LostGAN-V2 [87] | 42.55 | - | - |
| OC-GAN [89] | 41.65 | - | - |
| SPADE [62] | 41.11 | - | - |
...
| *LDM-4* (ours, 100 steps) | **2.8**$^\dagger$/**4.8**$^\ddagger$ | 166.3 | 24.4$_{\pm 3.8}$ | 0.69$_{\pm 0.14}$ |
| *LDM-4* (ours, 50 steps, guiding) | 4.4$^\dagger$/6.4$^\ddagger$ | 153.7 | 25.8$_{\pm 3.7}$ | 0.74$_{\pm 0.12}$ |
| *LDM-4* (ours, 100 steps, guiding) | 4.4$^\dagger$/6.4$^\ddagger$ | 154.1 | 25.7$_{\pm 3.7}$ | 0.73$_{\pm 0.12}$ |
| *LDM-4* (ours, 100 steps, +15 ep.) | **2.6**$^\dagger$ / **4.6**$^\ddagger$ | 169.76$_{\pm 5.03}$ | 24.4$_{\pm 3.8}$ | 0.69$_{\pm 0.14}$ |
| Pixel-DM (100 steps, +15 ep.) | 5.1$^\dagger$ / 7.1$^\ddagger$ | 163.06$_{\pm 4.67}$ | 24.1$_{\pm 3.3}$ | 0.59$_{\pm 0.12}$ |
...
**[Figure: Table of hyperparameters for unconditional LDMs]**
> **Caption:** Table 12. Hyperparameters for the unconditional LDMs producing the numbers shown in Tab. 1. All models trained on a single NVIDIA A100.
> **Figure text:** (Table content as rendered in image)

**[Figure: Table of hyperparameters for conditional LDMs]**
> **Caption:** Table 13. Hyperparameters for the conditional LDMs trained on the ImageNet dataset for the analysis in Sec. 4.1. All models trained on a single NVIDIA A100.
> **Figure text:** (Table content as rendered in image)
...
**[Figure: Hyperparameters for the experiments with transformer encoders]**
> **Caption:** Table 17. Hyperparameters for the experiments with transformer encoders in Sec. 4.3.
> **Figure text:** Text-to-Image Layout-to-Image seq-length 77 92 depth $N$ 32 16 dim 1280 512
